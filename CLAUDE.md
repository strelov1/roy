...
- **`crates/roy-cli`** — binary `roy`. Thin trigger over the daemon (Unix socket). The `roy mcp`, `roy gateway`, and `roy scheduler` subcommands delegate to the matching adapter crates, so a single binary covers every adapter.
