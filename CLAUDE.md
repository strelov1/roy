...
- **`crates/roy-mcp`** — library. MCP (Model Context Protocol) server. Hosts two subcommands:
  ...
  - **Connections MVP status (2026-05-27):** stdio upstream only; claude + opencode presets; plain-text secrets in DB (file mode 0600); tools snapshot at spawn (no `list_changed` push); resume does not re-attach connections; no `always_attach` flag. Track follow-ups under "Connections — User MCP Proxy" plan.
