...
- **`crates/roy-mcp`** — library. MCP (Model Context Protocol) server. Hosts two subcommands:
  ...
  - **Connections MVP status (2026-05-28):** stdio upstream only; claude + opencode + gemini presets; plain-text secrets in DB (file mode 0600); tools snapshot at spawn (no `list_changed` push); resume does not re-attach connections; no `always_attach` flag yet.
