...
- **`crates/roy-protocol`** — leaf wire crate. Owns the entire wire surface and nothing else: `control` (`ClientCommand`/`ServerEvent`/`ErrorCode`/`FireTarget`/`ConnectionSpec`), `event` (`TurnEvent`/`StopReason`/`event_to_json`/`event_from_json`), `journal` (`JournalEntry`/`Seq`/`parse_entry_line` — types only), `harnesses` (`Harness`/`HarnessInfo`/`ModelInfo`/`HarnessesConfigStatus` — wire types only), `pid_lock` (`PidLock` + helpers), and `wire` (`encode_line`/`decode_line`/`default_socket_path` — the single source of truth for the newline-delimited JSON framing + socket path). No tokio, ACP-SDK, or SQLite dependencies. `roy` core and every spoke crate depend on it.
- **`crates/roy`** — core library. Owns sessions: spawns ACP harness binaries over stdio, journals each turn, broadcasts events to N subscribers, persists boot-kit metadata in SQLite (`~/.local/state/roy/sessions.db`) so sessions survive across daemon restarts. Depends on `roy-protocol` and re-exports it at the historical `roy::...` paths.
...
- **`crates/roy-management`** — axum HTTP service for projects, session metadata, agent personas, and connections. Exposes `pub async fn run(args)`; `roy-cli` dispatches `roy management` to it. Owns `MetaStore` (`projects`, `session_meta`, `session_tags`, `connections`) on top of the shared `agents.db` SQLite file (`~/.local/state/roy/agents.db`, override with `ROY_AGENTS_DB`). Talks to the daemon over Unix socket via `DaemonClient` for session-coordination operations; routes project/tag/agent/connection operations directly to the database. It also surfaces read-only scheduler data (agents/triggers/fires) by opening roy-scheduler's `state.db` **read-only via `roy_scheduler::read::SchedulerRead` (no migrations applied)** — an explicit, documented cross-crate read dependency. Agent personas live as files in `.roy/agents/<slug>.md` (YAML frontmatter: `name`, `description`, `harness`, optional `model`; body becomes the session's system prompt). `POST /agents/_builder` spawns a session backed by an agent-builder persona that edits target agents via `roy agents update` CLI calls.
...
- **`crates/roy-inbound`** — inbound event bus for external systems (HTTP webhook and Telegram customer-support today; IMAP / WhatsApp remain roadmap). Pure publishers normalize external events into `InboundEvent`s onto an in-process `tokio::mpsc` bus; a single dispatcher resolves a per-source session strategy (`ephemeral`/`persistent_one`/`per_sender_sticky`), fires the agent over the daemon Unix socket, and a per-channel `ReplyHook` delivers the result back. Owns SQLite state at `~/.local/state/roy-inbound/state.db` (table `bindings`). Configured via TOML (`~/.config/roy/inbound.toml`). For the Telegram channel, bot tokens and agent bindings are managed in `roy-management` (`channel_bindings` table in `agents.db`); `roy-inbound` reads them at startup and on a 30 s poll via `GET /internal/telegram-sources` (bearer `ROY_INTERNAL_TOKEN`); each Telegram sender gets a `per_sender_sticky` session with the bound agent.

The spoke crates (`roy-mcp`, `roy-scheduler`, `roy-gateway`, `roy-management`, `roy-inbound`) depend on `roy-protocol`, not on `roy` core. The wire surface they may use is therefore exactly what `roy-protocol` exports — the boundary is now compiler-enforced rather than hand-maintained: a spoke physically cannot name `SessionManager`, `SessionEngine`, `Journal`, or `Transport` because the crate that defines them is not in its dependency graph. The Unix socket remains the only API. `roy-cli` is the exception — it depends on `roy` core because it hosts the daemon. `roy-auth` is a sibling library used by `roy-management` and `roy-gateway` for user/team storage and JWT verification; it does not depend on `roy` or `roy-protocol`.
