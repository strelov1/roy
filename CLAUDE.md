...
- **`crates/roy-management`** — library + thin binary `roy-management`. axum HTTP service for agent CRUD and starting sessions. Exposes `pub async fn run(args)` so `roy-cli` can dispatch `roy management` to the same code as the standalone binary. Same boundary rule as `roy-scheduler`/`roy-gateway`: talks to the daemon only over the Unix socket, passing `system_prompt = agent.prompt` inline on `Spawn`. Transitional note: `roy-scheduler` still has its own `agents` table until a future Plan C unifies it onto `roy-agents`.
