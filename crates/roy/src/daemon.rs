...
impl Daemon {
    ...
    async fn handle_fire(
        ...
        // Tags are accepted on the wire for forward compatibility but the
        // daemon no longer persists them — rich session metadata (tags,
        // project membership) lives in `roy-management`.
        // 1. Spawn or Resume
...
mod tests {
    ...
    async fn spawn_persists_system_prompt_in_metadata() {
        ...
        let row = daemon
            ...
            .await
            .unwrap()
            .expect("session row written by engine");
        assert_eq!(row.system_prompt.as_deref(), Some("PERSONA"));
